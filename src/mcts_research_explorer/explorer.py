...
class MCTSNode:
    ...
    # Exploration constant (UCB1 c parameter). sqrt(2) ≈ 1.414 is the textbook
    # default; raise to favor exploration of novel branches, lower to exploit
    # already-promising ones. Configurable so tuning per-topic is possible.
    c: float = 1.414
    ...
    def ucb1(self) -> float:
        ...
        (standard MCTS expansion semantics). Uses ``max(parent_visits, 1)``
        to guard against ``log(0)`` when the parent itself has not yet been
        visited (shouldn't happen in practice but defensive).
        """
        if self.visits == 0:
...
class MCTSResearchExplorer:
    ...
    def __init__(
        ...
        Args:
            ...
            max_nodes: Maximum nodes in tree (RAM bound for i5-4440).
    ...
    def _select(self, node: MCTSNode) -> Optional[MCTSNode]:
        ...
        if unexpanded:
            # UCB1 selection among unexpanded children
    ...
    def _simulate(self, node: MCTSNode):
        ...
        # Node quality = average CRAAP of its results
        if node.results:
    ...
    def _backpropagate(self, node: MCTSNode):
        ...
        while current is not None:
            if current.children:
                ...
                if child_visits > 0:
                    # Weighted average of own quality and children's quality.
                    # Use avg_quality (normalised [0,1]) not quality_sum (which
                    # scales with result count and can exceed 1.0), so the
                    # blend stays in [0,1] and UCB1 scores remain valid.
    ...
    def _default_expand(self, query: str, results: List[Dict]) -> List[str]:
        ...
        queries = []
        # Extract unique terms from top 3 result titles
    ...
    def get_tree_summary(self) -> str:
        ...
        def _render(node: MCTSNode, indent: int = 0):
            prefix = "  " * indent
            status = "✂️" if node.pruned else ("✅" if node.expanded else "⬜")
